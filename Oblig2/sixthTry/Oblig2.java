...
public class Oblig2{
	...
	public static void parallell(int maxNum){
		...
		int numsamount = (int) (Math.sqrt(num) / nrCores);
		rest = (int) Math.sqrt(num) % nrCores;

		es.initializefactorization(num);

		int startnum = 0;
		int endnum = numsamount;
		for(int i = 0; i < nrCores; i++){
			if(rest > 0){
				endnum++;
			}

			FactoriationThread ft = new FactoriationThread(i, cb, es, startnum, endnum);
			Thread t = new Thread(ft);
			t.start();

			startnum = endnum + 1;
			endnum = endnum + numsamount + 1;
		}

		ArrayList<Long> fakt = es.getFakt();
	...
	public static class FactoriationThread implements Runnable {
		int ind;
		CyclicBarrier cb;
		int startnum, endnum;
		EratosthenesSil es;

		public FactoriationThread(int ind, CyclicBarrier cb, EratosthenesSil es, int startnum, int endnum){
			this.ind = ind;
			this.cb = cb;
		...
		public void run(){
			es.factorizeParallell(startnum, endnum);
		...
		public ArrayList<Long> getFakt(){
		 ...
		 public synchronized void division(long divnum){
 		  ...
 		  public void factorizeParallell (int startnum, int endnum) {
				for(int idx = startnum; idx < endnum && idx <= Math.sqrt(currNum); idx++){
 					if(isPrime(idx) && currNum % idx == 0){
 						division((long) idx);
 						idx = startnum - 1;
 					}
 				}
